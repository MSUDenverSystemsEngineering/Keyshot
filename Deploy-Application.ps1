...
Try {
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		Show-InstallationWelcome -CloseApps 'keyshot, keyshot_daemon' -CheckDiskSpace -PersistPrompt
		...
		## Uninstall Keyshot 10
		If (Test-Path -LiteralPath (Join-Path -Path $envSystemDrive -ChildPath "$envProgramFiles\KeyShot10\uninstall.exe") -PathType 'Leaf') {
			Write-Log -Message 'Keyshot 10 was uninstalled during Pre-Install' -Source $deployAppScriptFriendlyName
		...
		## Uninstall Keyshot 11
		If (Test-Path -LiteralPath (Join-Path -Path $envSystemDrive -ChildPath "$envProgramFiles\KeyShot11\uninstall.exe") -PathType 'Leaf') {
		Write-Log -Message 'Keyshot 10 was uninstalled during Pre-Install' -Source $deployAppScriptFriendlyName
		Remove-File -Path "$envCommonDesktop\KeyShot 11.lnk"
		$exitCode = Execute-Process -Path "$envProgramFiles\KeyShot11\uninstall.exe" -Parameters "/S" -WindowStyle "Hidden" -WaitForMsiExec -PassThru
		If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
		}
		...
		$exitCode = Execute-Process -Path "$dirFiles\keyshot_win64_2023.1_12.0.0.186.exe" -Parameters '/S /AllUsers /FlexLM=27070@vmwas32.winad.msudenver.edu /NO_UPDATE /NO_CLOUD' -WindowStyle 'Hidden'
		If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
		...
		If (Test-Path "$env:Public\Desktop\Keyshot 2023.lnk") {
			Remove-Item "$env:Public\Desktop\Keyshot 2023.lnk" -Force
		}
		Else {
			Write-Log -Message "Shortcut not detected." -Source 'Pre-Installation' -LogType 'CMTrace'
		}
		If (Test-Path "$env:Public\Desktop\KeyShot 2023 Resources.lnk") {
			Remove-Item "$env:Public\Desktop\KeyShot 2023 Resources.lnk" -Force
		}
		Else {
			Write-Log -Message "Shortcut not detected." -Source 'Pre-Installation' -LogType 'CMTrace'
		}
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
		#>
		$exitCode = Execute-Process -Path "$envProgramFiles\KeyShot12\uninstall.exe" -Parameters "/S" -WindowStyle "Hidden" -WaitForMsiExec -PassThru
		Wait-Process -name Un_A
		If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }


		##MsiExec.exe /X{880BE667-C890-40D6-AD30-BB46089BCFE3}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBWbiZzB4ZhSW8/
# a8zeJm19svAN4Ad1UutBD+yZ31mSsaCCH8EwggVvMIIEV6ADAgECAhBI/JO0YFWU
...
# AYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAvBgkqhkiG9w0BCQQxIgQgvDeq1tKJaonj
# nZWGCQchMssG6qOV30V8l9fU07bJeo0wDQYJKoZIhvcNAQEBBQAEggGAEoJnQJWY
# UPNEE6ZtHVrLSzCK1wufHEUHvFBg1vLF5K/uR2ZRbmZ/wiKnLzwLiCvSvqPnqoP/
# 441rVXg5wU4hx/aBlKNBv4vuiq4IGMwGW3gMiTQ9bOpv2w+kn9FMP3zi2jAAwkrn
